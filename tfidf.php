...
$query = "SELECT f.id,f.kata,f.tf,g.idf,f.tf*g.idf tf_idf FROM (SELECT a.id,a.kata,a.freq,b.jmlkata,(a.freq/b.jmlkata) tf from (SELECT * FROM tfidf) AS a JOIN (SELECT id,SUM(freq) jmlkata FROM tfidf GROUP BY id) AS b ON a.id=b.id) AS f JOIN (SELECT d.kata,d.katajmldok,e.jmldok,LOG10(e.jmldok/d.katajmldok) idf from (SELECT kata,COUNT(kata) katajmldok FROM tfidf GROUP BY kata) AS d JOIN (SELECT COUNT(id) jmldok FROM (SELECT * FROM tfidf GROUP BY id) AS c) AS e) AS g ON f.kata=g.kata";
